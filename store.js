...
function cerrarModal(isPaypal = false) {
    if (!modalOverlay) return;
    if (isPaypal) {
      // Redirect to PayPal
...
document.querySelectorAll('.btn-comprar').forEach(btn => {
  ...
  /* Cerrar modal */
  if (modalCerrarBtn) modalCerrarBtn.addEventListener('click', () => cerrarModal(true));
  if (modalX)         modalX.addEventListener('click', () => cerrarModal(false));
  
  /* PayPal button */
  const modalPaypalBtn = document.getElementById('modal-paypal');
  if (modalPaypalBtn) modalPaypalBtn.addEventListener('click', (e) => {
    e.preventDefault();
    cerrarModal(true);
  });
